package com.android.lib.ticpod.fastble;
...
import com.android.lib.ticpod.fastble.bluetooth.BleBluetooth;
import com.android.lib.ticpod.fastble.bluetooth.MultipleBluetoothController;
import com.android.lib.ticpod.fastble.bluetooth.SplitWriter;
...
import com.android.lib.ticpod.fastble.callback.BleScanAndConnectCallback;
import com.android.lib.ticpod.fastble.callback.BleScanCallback;
import com.android.lib.ticpod.fastble.callback.BleWriteCallback;
import com.android.lib.ticpod.fastble.data.BleDevice;
import com.android.lib.ticpod.fastble.data.BleScanState;
import com.android.lib.ticpod.fastble.exception.OtherException;
import com.android.lib.ticpod.fastble.scan.BleScanRuleConfig;
import com.android.lib.ticpod.fastble.scan.BleScanner;
...
public class BleManager {
    ...
    private Map<String, Map> modelMap = new HashMap<>();
    ...
    public <K, V> Map<K, V> getMap(String model) {
        Map<K, V> map = modelMap.get(model);
        if (map != null) {
            return map;
        } else {
            map = new HashMap<>();
    ...
    public BluetoothGatt connect(BleDevice bleDevice, BleGattCallback bleGattCallback) {
        ...
        if (Looper.myLooper() == null || Looper.myLooper() != Looper.getMainLooper()) {
            LogUtil.w("Be careful: currentThread is not MainThread!");
        }
    ...
    public void write(BleDevice bleDevice,
        ...
        if (data.length > 20 && !split) {
            LogUtil.w("Be careful: data's length beyond 20! Ensure MTU higher than 23, or use spilt write!");
        }
    ...
    public void setMtu(BleDevice bleDevice,
        ...
        if (mtu > DEFAULT_MAX_MTU) {
            LogUtil.e("requiredMtu should lower than 512 !");
            callback.onSetMTUFailure(new OtherException("requiredMtu should lower than 512 !"));
            return;
        }

        if (mtu < DEFAULT_MTU) {
            LogUtil.e("requiredMtu should higher than 23 !");
    ...
    public BleDevice convertBleDevice(String mac) {
        BluetoothDevice bluetoothDevice = getBluetoothAdapter().getRemoteDevice(mac);
        return new BleDevice(bluetoothDevice);
    }
